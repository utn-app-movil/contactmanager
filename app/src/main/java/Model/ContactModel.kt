...
class ContactModel {
    ...
    fun contactExists(contact: Contact): Boolean {
        return dbManager.getAll().any { it.fullName == contact.fullName }
    }
    fun contactIdExists(id: String): Boolean {
        return dbManager.getAll().any { it.id == id }
    }
    fun contactPhoneExists(phone: Int): Boolean {
        return dbManager.getAll().any { it.phone == phone }
    }
}